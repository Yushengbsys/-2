...
public void OnPluginStart()
{
	...
	g_hCvarDelay = CreateConVar(				"l4d_mapchanger_delay",					"60",		"两个投票之间允许的最小延迟", CVAR_FLAGS );
	g_hCvarTimeout = CreateConVar(				"l4d_mapchanger_timeout",				"10",		"投票持续多长时间（秒）", CVAR_FLAGS );
	g_hCvarAnnounceDelay = CreateConVar(		"l4d_mapchanger_announcedelay",			"2.0",		"公告和投票菜单之间的延迟（秒）", CVAR_FLAGS );
	g_hCvarAllowDefault = CreateConVar(			"l4d_mapchanger_allow_default",			"1",		"是否显示默认地图菜单项？（1-是，0-否） (1 - Yes, 0 - No)", CVAR_FLAGS );
	g_hCvarAllowCustom = CreateConVar(			"l4d_mapchanger_allow_custom",			"1",		"是否显示自定义地图菜单项？（1-是，0-否）", CVAR_FLAGS );
	g_hCvarServerNameShort = CreateConVar(		"l4d_mapchanger_servername_short", 		"", 		"服务器的短名称（如果希望，请指定它，自定义活动名称将在其前面）", CVAR_FLAGS);
	...
	g_hCvarFinMapRandom = CreateConVar(			"l4d_mapchanger_fin_map_random", 		"0", 		"随机选择下一个自定义活动地图？（1-是，0-否）", CVAR_FLAGS);
	g_hCvarChapterList = CreateConVar(			"l4d_mapchanger_show_chapter_list", 	"1", 		"是否显示地图中的章节列表？（1-是，0-否）? (1 - Yes, 0 - No)", CVAR_FLAGS);
	g_hCvarFinaleChangeType = CreateConVar(		"l4d_mapchanger_finale_change_type", 	"8", 		"0-不更改最终章节（放置到大厅）；1-车辆离开时的瞬间；2-在决赛中瞬间获胜；4-等待信用屏幕出现；8-等待积分屏幕结束", CVAR_FLAGS);
	g_hCvarServerPrintInfo = CreateConVar(		"l4d_mapchanger_server_print_info", 	"1", 		"是否将地图更改信息显示到服务器控制台？（1-是，0-否）", CVAR_FLAGS);
	
	g_hCvarNativeVoteChangeMissionAllow = CreateConVar(	"l4d_native_vote_allow_change_mission", 	"0", 	"允许使用本地投票来改变任务？（0-否，替换为地图转换器菜单；1-是）。禁用它以提高安全性！", CVAR_FLAGS);
	g_hCvarNativeVoteChangeChapterAllow = CreateConVar(	"l4d_native_vote_allow_change_chapter", 	"0", 	"允许使用本地投票更改章节？（0-否，替换为地图转换器菜单；1-是）。禁用它以提高安全性！", CVAR_FLAGS);
	g_hCvarNativeVoteRestartGameAllow = CreateConVar(	"l4d_native_vote_allow_restart_game", 		"0", 	"允许使用本地投票重新启动游戏？（0-否，替换为地图转换器菜单；1-是）。禁用它以提高安全性！", CVAR_FLAGS);
